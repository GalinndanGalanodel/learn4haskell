...
--   | health monster <= strength knight = money knight + money monster
--   | health knight <= strength monster = (-1)
--   | otherwise = money knight
...
data Castle = NoCastle | Castle String | CastleAndWall String deriving (Show)
...
buildCastle city name = case castle city of
  NoCastle -> city {castle = Castle name}
  Castle _ -> city {castle = Castle name}
  CastleAndWall _ -> city {castle = CastleAndWall name}
...
buildWalls city
  | population >= 10 = case castle city of
    NoCastle -> city
    Castle name -> city {castle = CastleAndWall name}
    CastleAndWall _ -> city
  | otherwise = city
  where
    population = sum (map residents (houses city))
...
calculatePlayerHit (Damage dam) (Defense def) (Health h) = Health (h + def - dam)
...
--   maybe-treasure ;)
data Treasure x = Treasure
  { gold :: Int,
    loot :: x
  }

data Magic a = Magic a
